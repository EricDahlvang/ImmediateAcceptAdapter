...
using TestImmediateAcceptAdapter;
...
namespace ImmediateAcceptBot.BackgroundQueue
{
    ...
    public class HostedActivityService : BackgroundService
    {
        ...
        private readonly ConcurrentDictionary<ActivityWithAuthenticateRequestResult, Task> _activitiesProcessing = new ConcurrentDictionary<ActivityWithAuthenticateRequestResult, Task>();
        ...
        public HostedActivityService(IConfiguration config, IBot bot, ImmediateAcceptAdapter adapter, IActivityTaskQueue activityTaskQueue, ILogger<HostedActivityService> logger)
        {
            if (config == null)
            {
        ...
        private async Task BackgroundProcessing(CancellationToken stoppingToken)
        {
            ...
            {
                var activityWithAuthenticateRequestResult = await _activityQueue.WaitForActivityAsync(stoppingToken);
                if (activityWithAuthenticateRequestResult != null)
                {
                    try
                    {
                        ...
                        if (_lock.TryEnterReadLock(500))
                        {
                            ...
                            var task = GetTaskFromWorkItem(activityWithAuthenticateRequestResult, stoppingToken)
                                .ContinueWith(t =>
                            {
                            ...
                            _activitiesProcessing.TryAdd(activityWithAuthenticateRequestResult, task);
                        }
                        else
                        {
        ...
        private Task GetTaskFromWorkItem(ActivityWithAuthenticateRequestResult activityWithClaims, CancellationToken stoppingToken)
        {
                ...
                async () =>
            {
                try
                {
                    await _adapter.ProcessAuthenticatedActivity(activityWithClaims.AuthenticateRequestResult, activityWithClaims.Activity, _bot.OnTurnAsync, stoppingToken);
                }
                catch (Exception ex)
                {
